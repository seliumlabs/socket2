...
use crate::{Domain, MsgHdr, MsgHdrMut, Protocol, RecvFlags, SockAddr, TcpKeepalive, Type};
use libc::{c_uchar, c_void, in6_addr, in_addr, ssize_t};
use std::cmp::min;
use std::io::IoSlice;
...
pub(crate) use libc::{c_int, msghdr};
// Used in `Domain`.
pub(crate) use libc::{AF_INET, AF_INET6, AF_UNIX};
// Used in `Type`.
pub(crate) use libc::SOCK_RAW;
#[cfg(feature = "all")]
pub(crate) use libc::{IP_HDRINCL, SOCK_SEQPACKET};
...
// Used in `Socket`.
pub(crate) use libc::{
    ip_mreq as IpMreq, ip_mreq_source as IpMreqSource, ipv6_mreq as Ipv6Mreq, linger, IPPROTO_IP,
    IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, IPV6_DROP_MEMBERSHIP, IPV6_MULTICAST_HOPS,
    IPV6_MULTICAST_IF, IPV6_MULTICAST_LOOP, IPV6_RECVTCLASS, IPV6_UNICAST_HOPS, IPV6_V6ONLY,
    IP_ADD_MEMBERSHIP, IP_ADD_SOURCE_MEMBERSHIP, IP_DROP_MEMBERSHIP, IP_DROP_SOURCE_MEMBERSHIP,
    IP_MULTICAST_IF, IP_MULTICAST_LOOP, IP_MULTICAST_TTL, IP_RECVTOS, IP_TOS, IP_TTL, MSG_OOB,
    MSG_PEEK, SOL_SOCKET, SO_BROADCAST, SO_ERROR, SO_KEEPALIVE, SO_LINGER, SO_RCVBUF, SO_RCVTIMEO,
    SO_REUSEADDR, SO_SNDBUF, SO_SNDTIMEO, SO_TYPE, TCP_KEEPCNT, TCP_KEEPINTVL, TCP_NODELAY,
};
...
impl_debug!(Domain, libc::AF_INET, libc::AF_INET6);
...
pub(crate) fn poll_connect(socket: &crate::Socket, timeout: Duration) -> io::Result<()> {
    ...
    loop {
        ...
        let timeout = u128::clamp(timeout, 1, c_int::MAX as u128) as c_int;
